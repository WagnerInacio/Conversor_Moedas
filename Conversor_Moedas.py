...
while True:
 # 1-bloco : instruções
    ...
    while True:
        try:
            valor_inicial = float(
                input("Insira o valor desejado para operação:  \n"))
            break
        except ValueError:
            print(
                "\033[31m Opa! entrada invalida.   insira valor valido\33[0;0m"
            )
# 2-inserir moeda inicial
        ...
        try:
            ...
            if cod_moeda_inicial == 1:
                ...
                cod_moeda_inicial = input(
                    '\nOUTRO, codigo de moeda de escolha (3 digitos):'
                ).upper()
                print('Código de escolha do pais de operação: ', cod_moeda_inicial)
                break
            else:
...
# 3-inserir moeda para cambio
    titulo('SELECIONAR A MOEDA DE CAMBIO')
